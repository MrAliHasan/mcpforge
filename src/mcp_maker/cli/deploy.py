...
def _generate_dockerfile(server_file: str, port: int) -> str:
    ...
    # Derive autogen filename from server file (matches generator.py logic)
    import os as _os_deploy
    base_name = _os_deploy.path.splitext(_os_deploy.path.basename(server_file))[0]
    autogen_filename = f"_autogen_{base_name}.py"
