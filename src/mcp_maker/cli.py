...
def env(
    ...
    console.print()
    console.print(
        Panel.fit("🔐 [bold cyan]Environment Manager[/bold cyan]", subtitle="v" + __version__)
    )
...
def _env_set(env_file: str, key: str, value: str):
    ...
    console.print(f"\n  ✅ {action}: [cyan]{key}[/cyan] = [dim]{_mask_value(value)}[/dim]")
    console.print(f"  📁 Saved to: [green]{env_file}[/green]")

    if key in _KNOWN_VARS:
        console.print(f"  💡 {_KNOWN_VARS[key]}")
...
def _env_list(env_file: str):
    ...
    table = RichTable(title=f"🔐 Environment Variables ({env_file})")
...
def _env_delete(env_file: str, key: str):
    ...
    del env_vars[key]
    _env_write(env_file, env_vars)
    console.print(f"\n  ✅ Deleted: [cyan]{key}[/cyan] from {env_file}\n")
